...
- 综合评分：`100 * RankIC均值 + 8 * RankIC IR + 80 * Q5-Q1多空均值`，分数越高代表有效性、稳定性和组合收益的综合表现越好；排序默认按该分数降序。
- 拥挤度指标：市值相关、换手相关、Q5-Q1 PB 差、Q5-Q1 换手差及趋势。
- 个股展示：每个因子最新面板日按统一方向排序的前 20 只股票，只用于观察样本特征，不构成推荐。
- 当前不展示行业：用户已明确“不要行业了，个股就行，用已有的数据”。
- 因子说明：详情面板里的“构成与意义”会尽量说明因子公式、统一方向、投资含义和使用注意。基本面 Zoo 因子来自数据库公式；量价 Zoo 因子在数据库中未暴露底层公式，因此只按量价复合信号解释。
