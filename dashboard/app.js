...
const dataFiles = {
  ...
  weekly: "../outputs/factor_monitor_weekly.json",
};
...
const els = {
  ...
  industryExposure: document.getElementById("industryExposure"),
...
function renderIndustryExposure(detail) {
  const rows = detail.top_industry_exposure || [];
  if (!rows.length) {
    els.industryExposure.innerHTML = `<div class="empty-text">未接入行业映射，暂不显示行业分布</div>`;
    return;
  }
  const maxCount = Math.max(...rows.map((row) => row.count), 1);
  els.industryExposure.innerHTML = rows
    .map((row) => {
      const width = Math.max(8, (row.count / maxCount) * 100);
      return `
        <div class="exposure-row">
          <span>${row.industry}</span>
          <div class="exposure-track"><div style="width:${width}%"></div></div>
...
function renderTopStocks(detail) {
  ...
  if (!rows.length) {
    els.topStockRows.innerHTML = `<tr><td colspan="6" class="empty-cell">暂无最新因子值</td></tr>`;
    return;
  }
